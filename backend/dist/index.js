...
app.post("/api/project", async (c) => {
    ...
    try {
        ...
        const rawData = {
            ...
            googleDocs: form.get("googleDocs") || "",
            dropbox: form.get("dropbox") || "",
        };
        // Validate using Zod
        let validatedData;
        try {
            ...
            if (validationError instanceof z.ZodError) {
                const fieldErrors = validationError.issues.map((e) => `${e.path.join(".")}: ${e.message}`);
        ...
        if (files.length > MAX_FILES) {
            ...
            return c.json({ ok: false, message: `Maximum ${MAX_FILES} files allowed` }, 400);
        }
        // Validate file sizes
        let totalSize = 0;
        const validFiles = [];
        for (const file of files) {
            if (file.size > MAX_FILE_SIZE) {
                ...
                return c.json({ ok: false, message: `File "${file.name}" exceeds maximum size of ${MAX_FILE_SIZE / 1024 / 1024}MB` }, 400);
            }
            totalSize += file.size;
            if (totalSize > MAX_FILE_SIZE * 2) {
                console.warn(`[${requestId}] Total file size too large: ${totalSize} bytes`);
                return c.json({ ok: false, message: "Total file size exceeds limit" }, 400);
            }
        ...
        if (validatedData.googleDocs)
            console.log(`[${requestId}] Google Docs: ${validatedData.googleDocs}`);
        if (validatedData.dropbox)
            console.log(`[${requestId}] Dropbox:     ${validatedData.dropbox}`);
        ...
        return c.json({ ok: false, message: "An unexpected error occurred. Please try again later.", requestId: id }, 500);
    }
});
// 404 handler
app.notFound((c) => {
    ...
    return c.json({ ok: false, message: "Endpoint not found", requestId }, 404);
});
// Error handler
app.onError((err, c) => {
    let requestId = "unknown";
    ...
    return c.json({ ok: false, message: "An unexpected error occurred", requestId }, 500);
});
